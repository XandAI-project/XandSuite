"""
XandSuite Package: ComfyUI Image Generation
Generate images via a local ComfyUI instance using any image workflow.
...
# FastMCP server
# ---------------------------------------------------------------------------
mcp = FastMCP("xandsuite-comfyui-image")
...
def generate_image(
    ...
    """
    Generate an image using ComfyUI and the configured workflow.
    ...
    steps           Sampling steps. 0 = use connector default.
    ...
    # not use explicit placeholder tokens.
    ...
    # Poll /history until an image output appears or timeout.
    # Uses adaptive polling (starts fast, slows down) and tolerates transient
    ...
    while time.time() < deadline:
        ...
        # Scan output nodes for image outputs
...
def get_workflow_info() -> str:
    ...
    return json.dumps({
        ...
        "info": (
            ...
            "__STEPS__, __SEED__ in your workflow JSON."
        ),
    }, indent=2)
